...
`/unpaged:visual-plan` renders the plan in the conversation (or drafts one for a named feature) as a whiteboard on Unpaged: phases as connected boxes, each opening to its task checklist and exit criteria, risks on a note, a status stamp. The edit link comes back in the reply. The command then arms a receive-only listener for that one canvas. The listener collects an `@agent` comment on its next poll, and the session then acts with the Unpaged tools on that canvas and replies in the thread. Approving the plan in plan mode flips the stamp to EXECUTING through a PostToolUse hook, and from then on the session keeps a Decision log on the canvas: one row per deviation or non-obvious choice, written at the moment of the choice. When the code is done, `/unpaged:as-built` compiles the as-built record as a canvas nested under the plan — every plan item's outcome with its why, the decisions and where each why comes from (recorded during the work or reconstructed from the diff, labelled as such), the runtime flow that changed, and a reviewer's guide — and stamps the plan BUILT. `/unpaged:listen` shows, arms and revokes listeners per canvas.
...
- **Data sent:** the plan text and the elements drawn, the user's comments and the agent's replies, the Decision log rows the session writes while it implements, and — with `/unpaged:as-built` — file paths, commit subjects, diff summaries and the agent's description of the change (never file contents beyond a quoted identifier) — under the user's own account. No telemetry.
