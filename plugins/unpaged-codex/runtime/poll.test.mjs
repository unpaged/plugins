...
import { POLL_URL, EVENTS_URL, SUBPROTOCOL } from "./protocol.mjs";
import { MAX_POLL_EVENTS, pollInbox } from "./poll.mjs";

const SECRET = "private-test-listener-credential";
const binding = { documentId: "11111111-1111-4111-8111-111111111111", threadId: "22222222-2222-4222-8222-222222222222",
  keyId: "key-id", url: EVENTS_URL, pollUrl: POLL_URL, protocols: [SUBPROTOCOL, SECRET],
  codexPath: process.execPath, planDigest: "a".repeat(64) };
...
test("poll uses only the canonical endpoint and a header credential, returning stripped routing", async () => {
  const calls = [];
...
test("invalid endpoints and credentials fail before any request", async () => {
  ...
  for (const changes of [
    ...
    { pollUrl: POLL_URL.replace("https:", "http:") }, { protocols: [SUBPROTOCOL, `${SECRET}\nheader`] }
  ]) {
...
test("the entire envelope must validate before any routing is returned", async () => {
