...
- **Earlier records come first.** A task that an earlier as-built node on this canvas already carries as ✅ done or 🔀 changed keeps that outcome; its Why cell says *shipped in 📐 As built · <that date>* and it is not re-judged — a later round's range does not contain the earlier round's commits. Only tasks no earlier record resolved are judged against this range.
...
5. **Stamp the plan** (only these edits to the plan's own content, and only when the record is not partial):
   ...
   - Each phase box's label: prefix its `text` with the phase's outcome — ✅ (every task done), 🔀 (a task changed or dropped), or ⛔ (every task dropped) — `element_update` with `expectedRevision`, touching nothing else on the box.
