...
**Date** is today, `YYYY-MM-DD`. **Title** is `📐 As built · <date>`; a partial record is titled `📐 As built · <date> (partial)`. If a node with that exact title already exists on the plan canvas, append ` (2)`, then ` (3)`, and so on — `📐 As built · <date> (partial) (2)` is a second partial record of the same day. **Never edit a record from an earlier run** — not its title, not its elements: a re-run adds a record, it does not correct one. The record this run is writing is yours until the finish line; steps 1–4 build it in several calls.
...
5. **Stamp the plan** (only these edits to the plan's own content, and only when the record is not partial):
   ...
   - Each phase box's label: prefix its `text` with the phase's outcome — ✅ (every task done), 🔀 (a task changed or dropped), or ⛔ (every task dropped) — `element_update` with `expectedRevision`, touching nothing else on the box. Skip this whole step when the plan is already stamped `✅ BUILT`, and never prefix a box whose label already starts with ✅, 🔀 or ⛔ — the plan is stamped by the first complete record, and a later one leaves it as it is.
