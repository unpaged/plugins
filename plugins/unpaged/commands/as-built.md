...
1. **The plan.** `document_get(documentId)` gives every node and element: the root node (`rootNodeId`), the status stamp (the root's `text` element whose content starts with `**Status:**`), the phase boxes (the root `rectangle`s with `isLink`, in `linkOrder`, that link to a phase node — never the `📝 Decision log` box, never a `📐 As built · …` box an earlier run added: those are scaffolding, not phases of the plan), the phase nodes they link to and their `checklist` items (the plan's tasks, each with a stable item id), the Decision log node (title starts with `📝 Decision log`) and its table(s), and any earlier as-built nodes (title starts with `📐 As built`). A small single-phase plan has no phase node and no phase box: `/unpaged:visual-plan` puts its `checklist` on the root instead, and then the root's checklist items are the plan's tasks — one phase, with no box to label in step 5. A canvas whose tasks you find in neither place has no plan to reconcile: say *"No plan tasks on that canvas"* and stop, rather than recording every change as added work and stamping the plan BUILT.
...
6. **Link the record from the root**: a `rectangle` on the plan's root labelled `📐 As built · <date>` (with `(partial)` or `(2)` when the title has it) linking to the as-built node, on the row of the `📝 Decision log` box, to its right; the next record goes right of the previous one. A canvas rendered by an older plugin has no Decision log box: start the row below everything already on the root — the phase boxes, whether they run left-to-right or top-down, and the goal note — so a record never reads as a phase. Make room with `node_update` before placing a box that would not fit: `canvasWidth` when the row reaches the root's right edge, `canvasHeight` when the row itself falls below its bottom. A record box never sits outside the canvas and never overlaps an element already on it.
