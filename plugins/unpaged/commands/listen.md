...
- **No argument or `status`:** validate the stored key **without printing it** — this prints only `armed <keyId>` or `missing`:
  ...
  `monitor:connected` ⇒ say push is armed and listening. Otherwise arm the session Monitor yourself (step 3 of the arming steps) and say the background monitor was not running (if the reason was `no-websocket`, push needs Node 22 or newer on this machine). Not live (revoked elsewhere, or rejected with 4401) ⇒ `rm -f ~/.claude/unpaged/listener.json` and treat it as `missing`. If `missing`, arm it now: follow the "Arm the listener" steps of `/unpaged:visual-plan` (call `agent_listener_key_create` with label `claude-code on <hostname>`, store `{"url","protocols","keyId"}` atomically in `~/.claude/unpaged/listener.json` with mode 600, arm `Monitor({ ws: { url, protocols }, persistent: true, description: "UnPaged @agent comments" })` for this session). Never repeat the key in your reply.
