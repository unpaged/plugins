...
1. Check for a stored key **without printing it** — this validates the file and prints only `armed <keyId>` or `missing`:
   `node -e 'const f=process.env.HOME+"/.claude/unpaged/listener.json";try{const c=JSON.parse(require("fs").readFileSync(f,"utf8"));if(!/^wss?:\/\//.test(c.url)||!Array.isArray(c.protocols)||c.protocols.length<2)process.exit(2);console.log("armed "+(c.keyId||""))}catch{process.exit(2)}' 2>/dev/null || echo missing`
   Never `cat` that file — its contents are a credential and would land in the transcript. A malformed file counts as `missing` (`rm -f` it first). If it says `armed`, the plugin's background monitor already holds the socket for this session — tell the user *"I'm listening — comment @agent on the board and I reply there."* and stop here.
