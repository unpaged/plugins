...
4. Say what is actually true: if a session Monitor was armed in step 3 (or the background monitor reported `monitor:connected` in step 1), tell the user *"I'm listening — comment @agent on the board and I reply there."* If step 3 could not arm anything (Monitor tool unavailable, or the script could not be located), say instead *"Push is armed for your next session; this session isn't listening — comment @agent on the board and I'll sweep it next time, or ask me to check now."* Never claim to be listening without an armed Monitor.
