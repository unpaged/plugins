...
1. Check for a stored key **without printing it** — this validates the file and prints only `armed <keyId>` or `missing`:
   ...
   `monitor:connected` ⇒ the socket is open for this session: tell the user *"I'm listening — comment @agent on the board and I reply there."* and stop here. Anything else (`monitor:absent`, `monitor:dead`, `monitor:stopped`, `monitor:reconnecting`) ⇒ the background monitor is not holding the socket right now: go to step 3 and arm the session Monitor yourself with the stored `url`/`protocols` (read them with `node -e` into the Monitor call, never print them), and if the reason was `no-websocket` tell the user push needs Node 22 or newer on this machine.
