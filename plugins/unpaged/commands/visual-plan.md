...
2. **If `missing`**, call the `agent_listener_key_create` tool with `documentId` = this board and `label` = `claude-code on <host>` (the `host` line from step 1). **The plugin's PostToolUse hook stores the result** — `{"pollUrl","key","documentId","keyId","title","cwd","createdAt"}` at `~/.claude/unpaged/listeners/<documentId>.json`, mode 600, tmp + rename — the moment the tool returns, and tells you so in a context line that says *stored by the plugin hook*. Do not write the file yourself. Only if no such line arrived (an older Claude Code without plugin hooks, or the hook reported an error): run `node "${CLAUDE_PLUGIN_ROOT}/monitors/keys.mjs" store <documentId>` with the mint result as JSON on stdin (a heredoc — never on the command line, never `echo`); it prints `stored <keyId>`. Never repeat the key in your reply. If the tool refuses (a board you cannot open, or the 20-key cap), say so and stop arming.
3. **Arm this session on that board** by running the plugin's own script as a session Monitor with the board id as its argument (it reads the key file itself, never prints the key, and handles HTTP 401/409 on its own): `Monitor({ command: 'node "${CLAUDE_PLUGIN_ROOT}/monitors/listen.mjs" <documentId>', persistent: true, description: "Unpaged @agent comments — <board title>" })`. If the Monitor tool is unavailable in this session, skip this step.
4. Say what is actually true: after the Monitor has succeeded at its first authenticated poll (check `keys.mjs alive <documentId>` up to three times, roughly two seconds apart, stopping at `monitor:connected` or a terminal state), tell the user *"I'm listening on this canvas — comment @agent there and I reply on it."* If nothing could be armed (Monitor tool unavailable), say instead *"Push isn't armed in this session — comment @agent on the canvas and I'll sweep it when you ask, or run /unpaged:listen arm <documentId> in a session that can hold a Monitor."* If a Monitor exists but has not completed a successful poll, say it is starting or reconnecting. Never claim to be listening from process creation alone. Listening ends with this session; a later session re-arms with `/unpaged:listen`.
...
The Monitor is the plugin script, so it applies the HTTP result rules itself: `401` retires that board's invalid or revoked stored key without deleting a newer one; `409` stops because a newer key owns the board; `429`, `503` and network failures retry the same key. Its exit line tells you what happened. **Never mint a new key automatically after a terminal result**: if the line says the stored key file was retired, tell the user push is off for that canvas and that `/unpaged:listen arm <documentId>` turns it back on; if it says a newer key for the canvas is already stored by another session, or that a newer key superseded this key, do not remint. Check status before claiming another session is receiving comments.
