...
function validFrame(frame, documentId) {
...
export async function pollInbox(binding, { fetch: request = globalThis.fetch, signal,
  ...
  try {
    if (!isUnpagedListenerUrl(binding?.pollUrl) || !parseListenerConfig(JSON.stringify(binding))) throw failure("invalid_poll_configuration");
  ...
  const operation = async () => {
    ...
    const response = await request(url.href, {
      method: "GET", headers: { Authorization: `Bearer ${binding.key}`, Accept: "application/json" },
    ...
    const events = envelope.events.map((frame) => {
      ...
      if (!validFrame(frame, binding.documentId) || !frameLine(raw)) throw failure("poll_invalid_response");
      return frame;
    });
